...
class SurvedModel:
    ...
    def __init__(self):
        ...
        self.cl = { 'Weighted_BCEnDice_loss' : Weighted_BCEnDice_loss,
                      'mean_iou': mean_iou,
                      'dice_coef': dice_coef}
        self.model = tf.keras.models.load_model(filepath='C:/Users/Talha/Desktop/chkpt/brats/brats_model.h5',
    ...
    def predict (self, img):
        ...
        return o_pred
#%%
# img = np.load('C:/Users/Talha/Desktop/chkpt/brats/scan_1.npy')
# #############################################################
# #              Usage example with class
# #############################################################

# loaded_model = SurvedModel()
# op = loaded_model.predict(img)
...
#       'dice_coef': dice_coef}
# # Now load model
# model = tf.keras.models.load_model(filepath='C:/Users/Talha/Desktop/chkpt/brats/brats_model.h5',
#                                           custom_objects=cl, compile=True) 
# model.summary()
#%%
#############################################################
#              Usage example without class
#############################################################
# alpha = 0.5
# beta = (1.0 - alpha)
...
# preds_train_loaded = np.squeeze(model.predict(img, verbose=1))
# preds_train = (preds_train_loaded > 0.2).astype(np.uint8)
